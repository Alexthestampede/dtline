#!/usr/bin/env python3
"""
Test with start_width/height set to LATENT dimensions (64x64 for 512x512).
"""
...
# Build config WITH LATENT dimensions
builder = flatbuffers.Builder(256)

# Create string offset
model_offset = builder.CreateString('realdream_15sd15_q6p_q8p.ckpt')

# Build configuration with LATENT dimensions (64x64 for SD 1.5)
GenerationConfiguration.Start(builder)
GenerationConfiguration.AddId(builder, 0)
GenerationConfiguration.AddStartWidth(builder, 64)  # LATENT size, not pixel size!
GenerationConfiguration.AddStartHeight(builder, 64)
...
print(f'Config size: {len(config_bytes)} bytes')
print('Fields: start_width=64, start_height=64 (latent dimensions)')

# Create request - maybe scaleFactor=8 tells it to upscale 64->512?
request = imageService_pb2.ImageGenerationRequest(
    prompt='a basket full of kittens',
    configuration=config_bytes,
    scaleFactor=8,  # 64 * 8 = 512
    user='Test',
...
try:
    ...
    if generated_images:
        ...
        with open('output/kitten_latent_64.png', 'wb') as f:
            f.write(generated_images[0])
        print('✓ Saved to output/kitten_latent_64.png')
