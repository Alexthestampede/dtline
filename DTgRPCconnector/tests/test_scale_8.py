#!/usr/bin/env python3
"""
Test with start_width/height = 8 (scale factors, not pixels!)
"""
...
# Build config with SCALE FACTORS
builder = flatbuffers.Builder(256)

# Create string offset
model_offset = builder.CreateString('realdream_15sd15_q6p_q8p.ckpt')

# Build configuration with SCALE FACTORS (8 = 512px for SD 1.5)
GenerationConfiguration.Start(builder)
GenerationConfiguration.AddId(builder, 0)
GenerationConfiguration.AddStartWidth(builder, 8)  # Scale factor, not pixels!
...
print('Fields: start_width=8, start_height=8 (SCALE FACTORS)')
...
try:
    ...
    if generated_images:
        ...
        with open('output/kitten_scale8.png', 'wb') as f:
            f.write(generated_images[0])
        print('✓ Saved to output/kitten_scale8.png')
