#!/usr/bin/env python3
"""
Test WITHOUT original dimensions - only start (latent) and target (output).
"""
...
# Build configuration - start=latent, target=output, NO original
GenerationConfiguration.Start(builder)
GenerationConfiguration.AddModel(builder, model_offset)
GenerationConfiguration.AddStartWidth(builder, 64)    # Latent space
GenerationConfiguration.AddStartHeight(builder, 64)   # Latent space
# DO NOT set original dimensions (img2img only)
GenerationConfiguration.AddTargetImageWidth(builder, 512)   # Output size
GenerationConfiguration.AddTargetImageHeight(builder, 512)  # Output size
...
print('Testing WITHOUT original dimensions')
print('start=64 (latent), target=512 (output), NO original')
...
try:
    ...
    for response in client.stub.GenerateImage(request):
        if response.HasField('currentSignpost'):
            ...
            if signpost.HasField('sampling'):
                ...
                print(f'Image Decoded! (took {elapsed:.1f}s)')
    ...
    if generated_images:
        ...
        if elapsed < 15:
            print('✓ FAST generation (<15s) - This is the right config!')
        else:
            print('⚠ Still slow (>15s)')

        import os
        os.makedirs('output', exist_ok=True)
        with open('output/kitten_no_original.png', 'wb') as f:
            f.write(generated_images[0])
        print('✓ Saved to output/kitten_no_original.png')
